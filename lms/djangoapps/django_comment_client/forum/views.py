...
def get_user_graph_info(request, user_id):
    ...
    try:
        django_user_social = User.objects.get(id=user_id).social_auth.get(
            provider='azuread-oauth2')  # TODO: remove provider name hardcoding
        loggedin_user_social = request.user.social_auth.get(
            provider='azuread-oauth2')  # TODO: remove provider name hardcoding
        ...
        documents_modified_response = requests.get(
            sharepoint_site + "/_api/search/query?Querytext='*'&Properties='GraphQuery:ACTOR(" + actor_id + "\,action\:1003)'",
            headers={'Authorization': 'Bearer ' + access_token})
